...
it.each([
  ['simple', 'http://example.com', { queryParams: { sub: 'one' } }, 'http://example.com/simple?sub=one'],
  ['simple', 'http://example.com', { queryParams: { sub: ['one', 'two'] } }, 'http://example.com/simple?sub=one&sub=two'],
])('combines %s, %s and %s into %s', (url, base, parameters, expected) => {
  expect((new TemplateURL(url, base, parameters)).toString()).toBe(expected);
});

it.each([
  ['simple', 'http://example.com', { origin: 'https://mydomain.test' }, 'https://mydomain.test/simple'],
  ['simple', 'http://example.com/basepath', { origin: 'https://mydomain.test' }, 'https://mydomain.test/basepath/simple'],
  ['simple', 'http://example.com', { origin: 'https://localhost:8080' }, 'https://localhost:8080/simple'],
])('combines %s, %s and %s into %s', (url, base, parameters, expected) => {
  expect((new TemplateURL(url, base, parameters)).toString()).toBe(expected);
});
